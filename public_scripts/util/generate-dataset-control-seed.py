...
DATASET_SCREW_MAX_ATTEMPTS = 0
DATASET_RRT_MAX_ATTEMPTS = 3

# 固定目标模式：按下面的 FIXED_TARGETS 列表生成数据集。
# 为 True 时，CLI 的 --env/--episodes/--difficulty/--max-workers 仅保留兼容，不参与目标选择。
USE_FIXED_TARGETS = True
FIXED_TARGETS: List[Dict[str, Any]] = [

     {
      "env_id": "InsertPeg", "episode": 4, "seed": 13403, "difficulty": "easy"
    },
    {
      "env_id": "InsertPeg", "episode": 9, "seed": 13902, "difficulty": "easy"
    },
]
def _validate_fixed_targets(targets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    校验并标准化 FIXED_TARGETS。

    必须包含 env_id/episode/seed/difficulty。
    difficulty 仅支持 easy|medium|hard。
    禁止重复 (env_id, episode)。
    """
    if not targets:
        raise ValueError("FIXED_TARGETS is empty. Please provide at least one target.")

    normalized_targets: List[Dict[str, Any]] = []
    seen_env_episode: Set[Tuple[str, int]] = set()

    for idx, raw_target in enumerate(targets):
        if not isinstance(raw_target, dict):
            raise TypeError(f"FIXED_TARGETS[{idx}] must be a dict, got {type(raw_target).__name__}.")

        missing_keys = {"env_id", "episode", "seed", "difficulty"} - set(raw_target.keys())
        if missing_keys:
            raise ValueError(f"FIXED_TARGETS[{idx}] missing required keys: {sorted(missing_keys)}")

        env_id = str(raw_target["env_id"]).strip()
        if env_id not in ENV_ID_TO_CODE:
            raise ValueError(
                f"FIXED_TARGETS[{idx}] has unsupported env_id '{env_id}'. "
                f"Available: {sorted(ENV_ID_TO_CODE.keys())}"
            )

        try:
            episode = int(raw_target["episode"])
        except (TypeError, ValueError) as exc:
            raise ValueError(f"FIXED_TARGETS[{idx}] episode must be an integer.") from exc
        if episode < 0:
            raise ValueError(f"FIXED_TARGETS[{idx}] episode must be >= 0, got {episode}.")

        try:
            seed = int(raw_target["seed"])
        except (TypeError, ValueError) as exc:
            raise ValueError(f"FIXED_TARGETS[{idx}] seed must be an integer.") from exc
        if seed < 0:
            raise ValueError(f"FIXED_TARGETS[{idx}] seed must be >= 0, got {seed}.")

        difficulty_raw = raw_target["difficulty"]
        if not isinstance(difficulty_raw, str):
            raise ValueError(
                f"FIXED_TARGETS[{idx}] difficulty must be a string in {sorted(VALID_DIFFICULTIES)}."
            )
        difficulty = difficulty_raw.strip().lower()
        if difficulty not in VALID_DIFFICULTIES:
            raise ValueError(
                f"FIXED_TARGETS[{idx}] has invalid difficulty '{difficulty_raw}'. "
                f"Available: {sorted(VALID_DIFFICULTIES)}"
            )

        env_episode_key = (env_id, episode)
        if env_episode_key in seen_env_episode:
            raise ValueError(
                f"Duplicate (env_id, episode) found in FIXED_TARGETS: {env_episode_key}."
            )
        seen_env_episode.add(env_episode_key)
...
def run_env_fixed_targets(
    env_id: str,
    targets_for_env: List[Dict[str, Any]],
    temp_folder: Path,
    save_video: bool,
) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    固定目标模式：逐个 target 执行，每个 target 仅尝试一次，不重试。

    Returns:
        (success_records, failed_records)
    """
    temp_folder.mkdir(parents=True, exist_ok=True)
    temp_dataset_path = temp_folder / "temp_chunk.h5"

    success_records: List[Dict[str, Any]] = []
    failed_records: List[Dict[str, Any]] = []

    for target in targets_for_env:
        episode = int(target["episode"])
        seed = int(target["seed"])
        difficulty = str(target["difficulty"])

        print(
            f"[FixedTarget] env={env_id}, episode={episode}, seed={seed}, difficulty={difficulty}"
        )
            ...
            failed_records.append(
                {
                    ...
                    "seed": seed,
                    "difficulty": difficulty,
                    "error_or_status": f"exception: {exc}",
                }
            )
            print(
                f"[FixedTarget] Exception for env={env_id}, episode={episode}, seed={seed}: {exc}"
            )
            continue
            ...
            failed_records.append(
                {
                    ...
                    "seed": seed,
                    "difficulty": difficulty,
                    "error_or_status": "episode_failed_no_retry",
                }
            )
...
def _save_episode_metadata(
    ...
    sorted_failed_records = sorted(failed_records, key=lambda rec: rec.get("episode", -1))
    metadata = {
        "env_id": env_id,
        "success_count": len(sorted_records),
        "failed_count": len(sorted_failed_records),
        "record_count": len(sorted_records),
        "records": sorted_records,
        "failed_records": sorted_failed_records,
    }
    try:
...
def main() -> None:
    args = parse_args()
    _ = args.dataset_dir  # 保留参数兼容，不在当前脚本路径逻辑中使用

    if USE_FIXED_TARGETS:
        normalized_targets = _validate_fixed_targets(FIXED_TARGETS)
        targets_by_env: Dict[str, List[Dict[str, Any]]] = {}
        for target in normalized_targets:
            targets_by_env.setdefault(target["env_id"], []).append(target)

        for env_id, env_targets in sorted(targets_by_env.items()):
            env_targets = sorted(env_targets, key=lambda t: t["episode"])
            ...
            # 固定模式每次清理旧 h5，确保结果只包含本次固定目标
            if final_dataset_path.exists():
                final_dataset_path.unlink()
                print(f"Removed existing dataset file: {final_dataset_path}")

            # 清理历史临时目录，避免混入旧 episode
            if temp_folder.exists():
                shutil.rmtree(temp_folder)
                print(f"Removed existing temp folder: {temp_folder}")

            print(f"\n{'='*80}")
            print(f"[FixedTarget] Environment: {env_id}")
            print(f"[FixedTarget] Target count: {len(env_targets)}")
            print("[FixedTarget] No retry; continue on failure.")
            print(f"[FixedTarget] Temporary folder: {temp_folder}")
            print(f"[FixedTarget] Final dataset: {final_dataset_path}")
            print(f"{'='*80}\n")

            success_records, failed_records = run_env_fixed_targets(
                env_id=env_id,
                targets_for_env=env_targets,
                temp_folder=temp_folder,
                save_video=args.save_video,
            )

            print(f"\nMerging fixed targets into final dataset...")
            ...
            print(
                f"\n✓ Finished {env_id}: success={len(success_records)}, "
                f"failed={len(failed_records)}. Final dataset: {final_dataset_path}\n"
            )

        print("✓ All fixed targets processed.")
        return
