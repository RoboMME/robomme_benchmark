# -*- coding: utf-8 -*-
# 脚本功能：按 episode 运行 oracle_planner 评测（不读取 h5 数据集）
# 特点：跳过 demonstration 阶段后执行 dummy action（no-op command_dict），并保留 obs/info 展开逻辑；无 video。
...
def _get_dummy_action_oracle(available_options):
    """从 available_options 取第一项作为 action，坐标固定为 (0, 0)。"""
    if not available_options:
        return {"action": None, "point": None}
    first_opt = available_options[0]
    action_label = first_opt.get("action", "") if isinstance(first_opt, dict) else str(first_opt)
    return {"action": action_label, "point": [0, 0]}


def main():
    """
    主流程：按 episode 运行评测，不读取 h5 数据集。
    - 每个 episode 在 demonstration 结束后执行 dummy action（oracle no-op）。
    ...
    for env_id in env_id_list:
        ...
        for episode in range(50):
            ...
            # ---------- 按 step 运行：action 取 available_options 第一项，坐标 (0,0) ----------
            step = 0
            while step < max_steps:
                dummy_action = _get_dummy_action_oracle(available_options)
