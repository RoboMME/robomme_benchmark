...
def run_visual_prep_step_a(env, solve_options, selected_target, env_id, color_map, use_segmentation=False):
    ...
    This generates an internal temporary state to produce the image, but does not expose logic state to main.
    """
...
def run_input_step_b(solve_options, img_a):
    """
    Step B: Input Collection Only.
    Displays options and collects input. No logic processing or state reading here.
    """
    ...
    if MOUSE_SELECT:
        ...
        while _click_coords is None:
            cv2.imshow(mouse_win_name, img_a) 
            key = cv2.waitKey(20)
            if key == 27: 
                break
    ...
    # 3. Confirmation (CMD1 repeated to confirm in the logic engine)
    automation_cmds.append(cmd1)
...
def run_solve_and_evaluate(env, solve_options, selected_target, env_id, vis_context, automation_cmds):
    ...
    Reconstructs logic state from automation_cmds internally (starting fresh), validates selection, and executes.
    """
    ...
    # We pass external_state=None so it rebuilds state from the inputs [Cmd1, Cmd2, Cmd1]
    ...
    for env_id in env_id_list:
        ...
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for episode in range(num_episodes):
                ...
                try:
                    while True:
                        # [Step A] Visual Prep (No GUI state passed/returned)
                        ...
                        # [Step B] Input Collection (No GUI state passed)
                        ...
                        # [Step C] Process Logic & Solve (Internal State Only)
