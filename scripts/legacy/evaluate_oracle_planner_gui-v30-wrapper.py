...
# Helper variable for mouse callback
_click_coords = None
...
# =============================================================================
# Refactored Functions for Main Loop
# =============================================================================

def run_visual_prep_step_a(env, solve_options, selected_target, env_id, color_map, use_segmentation=False):
    """
    Handles fetching observations, preparing visualization, and rendering the initial state (Step A).
    """
    ...
    gui_state, img_a = _prompt_next_task_gui(
        ...
        simulated_inputs=[] # Empty action for initial state
    )

    fig_a = _display_step_image("Step A: Initial State", img_a)
    
    # Pack context data needed for Step B
    vis_context = {
        "seg_vis": seg_vis,
        "seg_raw": seg_raw,
        "base_frames": base_frames,
...
def run_input_step_b(env, solve_options, gui_state, selected_target, env_id, vis_context, img_a):
    """
    Handles user input (Options -> Mouse/Coords -> Confirm) and renders the annotated state (Step B).
    """
    # Unpack context
    ...
    if MOUSE_SELECT:
        ...
        win_name = "Select Target (Mouse)"
        cv2.namedWindow(win_name)
        cv2.setMouseCallback(win_name, _on_mouse_click)
        
        # Loop until click or ESC
        while _click_coords is None:
            cv2.imshow(win_name, img_a) # Use the image from Step A
            key = cv2.waitKey(20)
    ...
    # 3. Confirmation (re-using cmd1 as per original logic)
    automation_cmds.append(cmd1)

    print(f"\n[Step B] Executing Sequence: {automation_cmds}")
    
    # Update GUI State with inputs
    gui_state, img_b = _prompt_next_task_gui(
        solve_options,
        env,
        selected_target,
        env_id,
    ...
    fig_b = _display_step_image("Step B: After Commands", img_b)

    return gui_state, fig_b


def run_solve_and_evaluate(env, solve_options, gui_state):
    """
    Executes the solver based on the selection in gui_state and evaluates the environment.
    """
    selection_idx = gui_state.get("selection")
    
    if selection_idx is not None:
        print(f"\n[Step C] Execution. Selected Option: {selection_idx + 1}")
        ...
        print("\n[Step C] Error: No selection was confirmed.")

    # Final Evaluation for the loop
    ...
    return evaluation, fail_flag, success_flag
            ...
            for episode in range(num_episodes):
                ...
                try:
                    while True:
                        ...
                        # --- Function 2: Input Collection & Step B ---
                        gui_state, fig_b = run_input_step_b(
                            env, 
                            solve_options, 
                            gui_state, 
                            selected_target, 
                        ...
                        # --- Function 3: Solve & Evaluate ---
                        evaluation, fail_flag, success_flag = run_solve_and_evaluate(
                            env, 
                            solve_options, 
                            gui_state
                        )
