...
def run_env_dataset(
    ...
    Returns:
        Path to the temporary folder containing episode files
    """
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Environment ID to run. Defaults to the built-in HistoryBench environment.",
    )
    parser.add_argument(
        "--episodes",
        "-n",
...
def main() -> None:
    args = parse_args()
    env_id = args.env or DEFAULT_ENV
    ...
    # temp_folder = dataset_dir / f"temp_{env_id}_episodes"
    # final_dataset_path = dataset_dir / f"record_dataset_{env_id}.h5"
    ...
    if num_workers > 1:
        ...
        if len(episode_chunks) <= 1:
            ...
            run_env_dataset(
                env_id,
                episode_chunks[0] if episode_chunks else [],
                temp_folder,
                args.save_video,
            )
        else:
            ...
            with ProcessPoolExecutor(max_workers=worker_count) as executor: 
                future_to_chunk = {
                    ...
                    for idx, chunk in enumerate(episode_chunks)
                }

                for future in as_completed(future_to_chunk):
                    _, chunk = future_to_chunk[future]
