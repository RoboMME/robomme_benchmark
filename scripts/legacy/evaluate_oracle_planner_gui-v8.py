...
def _prompt_next_task_gui(
    ...
    def _collect_available_targets():#可用点
        """
        Gather candidate actors from option-provided 'available' lists first,
        fallback to env.spawned_cubes. Filters out None and deduplicates by id.
        """
        candidates = []
        seen = set()
        for opt in options:
        ...
        if seg_rect and seg_raw is not None:
            ...
            if x1 <= x <= x2 and y1 <= y <= y2:
                ...
                if 0 <= sx < seg_w and 0 <= sy < seg_h:
                    ...
                    available_targets = _collect_available_targets()
                    click_disp = (x - x1, y - y1)
                    ...
                    if chosen is not None:
                        ...
                        # 使用质量中心点显示标记
                        selected_target["click_point"] = chosen["disp_point"]
                        print(
