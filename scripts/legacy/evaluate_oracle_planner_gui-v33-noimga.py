...
def run_visual_prep_step_a(env, solve_options, selected_target, env_id, color_map, use_segmentation=False):
    ...
    Generates internal state to produce the image but returns only context and display image.
    """
    ...
    # Safety fallback: Ensure seg_vis is valid for vis_context so Step B doesn't crash
    if seg_vis is None:
    ...
    # Pack context data needed for Step B and Step C
    vis_context = {
        "seg_vis": seg_vis,       # Clean image for background in Step B
        "seg_raw": seg_raw,       # Raw ID map for logic in Step C
        "base_frames": base_frames,
        "wrist_frames": wrist_frames,
    }

    return img_a, fig_a, vis_context


def run_input_step_b(solve_options, vis_context):
    """
    Step B: Input Collection Only.
    Uses 'vis_context' to provide the background image for mouse selection.
    """
    _print_terminal_options(solve_options)
    automation_cmds = []
    mouse_win_name = "Select Target (Mouse)"
    
    # Extract the clean image from context for mouse interaction background
    reference_img = vis_context["seg_vis"]
    ...
    if MOUSE_SELECT:
        ...
        while _click_coords is None:
            # Use the clean reference image from context
    ...
    # 3. Confirmation (CMD1 repeated for logic confirmation)
    automation_cmds.append(cmd1)

    return automation_cmds, mouse_win_name


def run_solve_and_evaluate(env, solve_options, selected_target, env_id, vis_context, automation_cmds):
    """
    Step C: Logic Processing & Execution.
    Reconstructs logic state from automation_cmds internally using vis_context data.
    """
        ...
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for episode in range(num_episodes):
                ...
                try:
                    while True:
                        # [Step A] Visual Prep
                        img_a, fig_a, vis_context = run_visual_prep_step_a(
                            env, 
                            solve_options, 
                            selected_target, 
                            env_id, 
                        ...
                        # [Step B] Input Collection (Using vis_context for background)
                        automation_cmds, fig_b = run_input_step_b(
                            solve_options, 
                            vis_context   # Passing context instead of img_a
                        )
                        if fig_b:
                            figures_to_close.append(fig_b)

                        # [Step C] Process Logic & Solve
