...
def run_visual_prep_step_a(env, env_id, color_map, use_segmentation=False, use_visualize=True):
    """
...
def run_input_step_b(env, planner, env_id, seg_vis, mouse_select=True, use_visualize=True):
    """
    Step B: Input Collection Only.
    Generates temporary options locally just to display the menu and check validity.
    """
...
def run_solve_and_evaluate(env, planner, env_id, seg_vis, seg_raw, base_frames, wrist_frames, automation_cmds, use_visualize=True):
    """
    Step C: Logic Processing & Execution.
    Initializes selected_target, generates bound options, and executes logic internally.
    """
    
    # --- INTERNAL STATE INITIALIZATION ---
    # selected_target is defined ONLY here
    selected_target = {
        "obj": None,
        "name": None,
        "seg_id": None,
    ...
    for env_id in env_id_list:
        ...
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for episode in range(num_episodes):
                ...
                try:
                    while True:
                        # [Step A] Visual Prep - No selected_target needed here
                        ...
                        #[Step B] Input Collection - Generates local temporary options for menu
                        ...
                        # [Step C] Process Logic & Solve - Manages selected_target internally
                        evaluation, fail_flag, success_flag, fig_c = run_solve_and_evaluate(
                            env, 
                            planner, # Pass planner
                            env_id, 
                            seg_vis,
                            seg_raw,
                            base_frames,
