...
def run_visual_prep_step_a(env, solve_options, selected_target, env_id, color_map, use_segmentation=False, visualize=True):
    """
    Step A: Visualization Preparation & Initial Render.
    Generates internal state and returns individual data components.
    """
    ...
    # Using internal temporary state (external_state=None) for rendering
    ...
    # Return explicit parameters instead of a dictionary
    return  fig_a, seg_vis, seg_raw, base_frames, wrist_frames


def run_input_step_b(solve_options, seg_vis, mouse_select=True, visualize=True):
    """
    Step B: Input Collection Only.
    Returns [Option, Coords].
    """
    ...
    # --- LOGIC UPDATE: Check availability before asking for coordinates ---
    skip_coords = False
    try:
        ...
        if 0 <= idx < len(solve_options):
            ...
            # If 'available' is None or an empty list/dict, skip coordinate selection
            if not available_targets:
                print(f"  -> Option {idx + 1} has no specific targets available.")
                print("  -> Skipping coordinate selection (Auto-set to '0 0').")
                skip_coords = True
    except ValueError:
        # If input wasn't a number, proceed as normal (or handle error as preferred)
        pass
    # --------------------------------------------------------------------
    ...
    if skip_coords:
        ...
        if mouse_select and visualize:
            ...
            if click_state["coords"]:
                cmd2 = f"{click_state['coords'][0]} {click_state['coords'][1]}"
    ...
    # REMOVED: automation_cmds.append(cmd1) -- Confirmation now handled in Step C

    return automation_cmds, mouse_win_name


def run_solve_and_evaluate(env, solve_options, selected_target, env_id, seg_vis, seg_raw, base_frames, wrist_frames, automation_cmds, visualize=True):
    """
    Step C: Logic Processing & Execution.
    Expands input to [Option, Coords, Option] before processing.
    """
    
    # --- AUTO-FILL CONFIRMATION ---
    # Convert ['1', '0 0'] -> ['1', '0 0', '1']
    processing_cmds = list(automation_cmds)
    if len(processing_cmds) > 0:
        processing_cmds.append(processing_cmds[0])
    
    print(f"\n[Step C] Processing Sequence: {processing_cmds}")
    ...
    final_internal_state, img_c = _prompt_next_task_gui(
        ...
        base_frames=base_frames,
        wrist_frames=wrist_frames,
        simulated_inputs=processing_cmds,
        external_state=None
    )
    ...
    for env_id in env_id_list:
        ...
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for episode in range(num_episodes):
                ...
                try:
                    while True:
                        # [Step A] Visual Prep - Visualization Optional
                        fig_a, seg_vis, seg_raw, base_frames, wrist_frames = run_visual_prep_step_a(
                            env, 
                            solve_options, 
                            selected_target, 
                            env_id, 
                        ...
                        # # [Step B] Input Collection - Visualization Optional
                        ...
                        # AUTO TESTING OVERRIDE: Updated to only include [Option, Coords]
                        #automation_cmds=['1', '0 0'] 

                        # [Step C] Process Logic & Solve - Visualization Optional
                        # run_solve_and_evaluate will automatically append the confirmation input
