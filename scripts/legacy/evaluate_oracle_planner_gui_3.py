...
def get_obs_and_segmentation(env, planner, env_id, color_map, use_segmentation=False, use_visualize=True):
    ...
    Returns:
       fig_a (window name or None),
       seg_vis (image array),
       seg_raw (raw segmentation),
       base_frames (list),
       wrist_frames (list),
       available_options (list of dicts: [{"action": "Name", "need_parameter": bool}, ...])
    """
...
def get_input_from_gui(env, planner, env_id, seg_vis, mouse_select=True, use_visualize=True):
    ...
    Returns dictionary: {"action": "Name", "point": (x, y)} or None point.
    """
...
def solve_and_evaluate_from_command(env, planner, env_id, seg_vis, seg_raw, base_frames, wrist_frames, command_dict, use_visualize=True):
    ...
    if "action" not in command_dict or "point" not in command_dict:
        print("Error: Invalid command format. Expected format: {'action': 'ActionName', 'point': (x, y) or None}")
        print(f"Received: {command_dict}")
        return None, None

    if target_action is None or not isinstance(target_action, str):
        print("Error: 'action' must be a non-empty string.")
        print(f"Received action: {target_action}")
        return None, None
    ...
    for env_id in env_id_list:
        ...
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for episode in range(num_episodes):
                ...
                try:
                    while True:
                        # [Step A] Visual Prep AND Option Generation
                        ...
                        if evaluation is None:
                            print("Skipping evaluation due to invalid command format.")
                            continue
