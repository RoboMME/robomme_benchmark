...
MOUSE_SELECT = True  # Set to True to use Mouse for the 2nd command (coordinates)
...
def _cleanup_figures(figures, close_all=False):
    """Close OpenCV preview windows and tear down GUI state."""
    if figures:
...
# --- Core Logic Function (Used by Step A and Step C) ---
def _prompt_next_task_gui(
    ...
    This function processes the logic of matching coordinates to objects.
    """
            ...
            if x1 <= global_x <= x2 and y1 <= global_y <= y2:
                ...
                if 0 <= sx < seg_w and 0 <= sy < seg_h:
                    ...
                    if pending_option is None:
                        # Auto arming logic could go here if needed, but we rely on explicit option selection
                        return
                    ...
                    if chosen is not None:
                        ...
                        # Fallback: Just select whatever is under the cursor
...
def run_visual_prep_step_a(env, solve_options, selected_target, env_id, color_map, use_segmentation=False):
    """
    Step A: Visualization Preparation & Initial Render
    - Fetches observations.
    - Renders the initial state image (img_a).
    - Returns a context dict containing raw data for Step C.
    """
    ...
    if use_segmentation:
        ...
        # Generate seg_raw but use RGB for visualization
        _, seg_raw = (
    ...
    # Empty inputs for initial render
    gui_state, img_a = _prompt_next_task_gui(
        solve_options,
        env,
        selected_target,
        env_id,
    ...
    # Pack context data needed for Step C
    vis_context = {
        "seg_vis": seg_vis,
        "seg_raw": seg_raw,
        "base_frames": base_frames,
    ...
    return gui_state, img_a, fig_a, vis_context


def run_input_step_b(solve_options, gui_state, img_a):
    """
    Step B: Input Collection Only
    - Displays options and waits for user input.
    - If MOUSE_SELECT is True, opens a window to capture clicks on img_a.
    - Returns a list of command strings (automation_cmds) and the mouse window name.
    - DOES NOT run any logic or update target state.
    """
    _print_terminal_options(solve_options, gui_state)
    automation_cmds = []
    mouse_win_name = "Select Target (Mouse)"

    print("\n[Input] Collecting Commands...")
    
    # 1. Option Selection
    cmd1 = input("  Command 1 (Option selection, e.g. '1'): ")
    automation_cmds.append(cmd1)
    ...
    if MOUSE_SELECT:
        ...
        # Open window for mouse interaction
        cv2.namedWindow(mouse_win_name)
        cv2.setMouseCallback(mouse_win_name, _on_mouse_click)
        
        # Loop until click or ESC
        while _click_coords is None:
            cv2.imshow(mouse_win_name, img_a) # Use the image from Step A
            key = cv2.waitKey(20)
            if key == 27: # ESC to cancel/skip
                break
        
        # Note: We do NOT destroy the window here, we pass the name back to be cleaned up in main
        
        if _click_coords:
            cmd2 = f"{_click_coords[0]} {_click_coords[1]}"
            print(f"  -> Captured Coordinates: {cmd2}")
        else:
            print("  -> No click detected. Defaulting to '0 0'")
            cmd2 = "0 0"
    else:
        cmd2 = input("  Command 2 (Coords, e.g. '150 150'): ")
        mouse_win_name = None # No window used

    automation_cmds.append(cmd2)

    # 3. Confirmation (re-using cmd1 as per original logic to confirm selection)
    automation_cmds.append(cmd1)

    return automation_cmds, mouse_win_name


def run_solve_and_evaluate(env, solve_options, selected_target, env_id, vis_context, automation_cmds, gui_state):
    """
    Step C: Logic Processing & Execution
    - Takes automation_cmds from Step B.
    - Calls _prompt_next_task_gui to parse commands and update selected_target.
    - Renders the confirmation image (img_c).
    - Executes the solver.
    """
    
    # Unpack context
    seg_vis = vis_context["seg_vis"]
    seg_raw = vis_context["seg_raw"]
    base_frames = vis_context["base_frames"]
    wrist_frames = vis_context["wrist_frames"]

    print(f"\n[Step C] Processing Sequence: {automation_cmds}")

    # --- 1. Process Logic & Update State ---
    # This is where selected_target gets updated based on the coords in automation_cmds
    updated_gui_state, img_c = _prompt_next_task_gui(
        solve_options,
        env,
        selected_target,
        env_id,
        ...
        base_frames=base_frames,
        wrist_frames=wrist_frames,
        simulated_inputs=automation_cmds,
    ...
    # --- 3. Execution ---
    selection_idx = updated_gui_state.get("selection")
    ...
    # --- 4. Evaluation ---
    env.unwrapped.evaluate() # Standard eval update
    ...
    return evaluation, fail_flag, success_flag, fig_c
    ...
    for env_id in env_id_list:
        ...
        with h5py.File(dataset_path, "r") as dataset:
            ...
            for episode in range(num_episodes):
                ...
                # Container for target info, updated across steps
                selected_target = {
                    "obj": None,
                    "name": None,
                    "seg_id": None,
                ...
                try:
                    while True:
                        # ============================================================
                        # [Step A] Visual Prep & Initial Render
                        # ============================================================
                        # Get Observation -> Render -> Return GUI State & Context
                        gui_state, img_a, fig_a, vis_context = run_visual_prep_step_a(
                            env, 
                            solve_options, 
                            selected_target, 
                            env_id, 
                        ...
                        # ============================================================
                        # [Step B] Input Collection (No Logic Processing)
                        # ============================================================
                        # Interaction -> Return Commands List & Window Name
                        automation_cmds, fig_b = run_input_step_b(
                            solve_options, 
                            gui_state, 
                            img_a
                        )
                        ...
                        # ============================================================
                        # [Step C] Process Logic, Confirm, Solve & Evaluate
                        # ============================================================
                        # Commands + Context -> Update Logic -> Render Confirm -> Execute
                        evaluation, fail_flag, success_flag, fig_c = run_solve_and_evaluate(
                            env, 
                            solve_options, 
                            selected_target, 
                            env_id, 
                            vis_context,      # Passed from Step A
                            automation_cmds,  # Passed from Step B
                            gui_state
                        )
