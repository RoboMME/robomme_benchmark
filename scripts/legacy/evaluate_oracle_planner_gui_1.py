...
def solve_and_evaluate_from_command(env, planner, env_id, seg_vis, seg_raw, base_frames, wrist_frames, automation_cmds, use_visualize=True):
    """
                ...
                try:
                    while True:
                        ...
                        fig_a, seg_vis, seg_raw, base_frames, wrist_frames = get_obs_and_segmentation(
                            env, 
                            env_id, #pass env_id for options
                            color_map, #for segmentation visualization
                        ...
                        command, fig_b = get_input_from_gui(
                            env,
                            planner,
                            env_id, #pass env_id for options
                            seg_vis,   #pass seg_vis for display
                        ...
                        evaluation, fig_c = solve_and_evaluate_from_command(
                            ...
                            env_id, #pass env_id for options
                            seg_vis,#pass seg_vis for display
                            seg_raw,#pass seg_raw for segmentation logic
                            base_frames,#history of base camera frames
                            wrist_frames,#history of wrist camera frames
                            command,
                            use_visualize=use_visualization
                        )
                        if fig_c:
