...
def _compose_quadrant_grid(env, frame_idx, target_hw=(240, 320), segmentation_bgr=None):
    """
    将当前环境帧拼成 2x2 方格用于预览。
    - 左列：按 frame_idx 播放的视频帧（base / wrist）
    - 右列：默认最新帧（base / wrist），若提供 segmentation_bgr 则右上显示分割图
...
def _prompt_next_task_gui(
    ...
    while True:
        ...
        cv2.putText(
            grid,
            "Top: segmentation (click target). Middle: playback history (base | wrist).",
            (10, y),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (0, 255, 0),
        ...
        cv2.putText(
            combined,
            "Click segmentation to pick target cube, then use buttons/number keys to run solve. Space pause, q/Esc exit.",
            (10, y + 24),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (0, 255, 0),
        ...
        cv2.putText(
            combined,
            f"Selected target: {selected_name}",
            (10, y + 44),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.55,
            (0, 255, 0),
    ...
    for env_id in env_id_list:
        for episode in range(num_episodes):
            ...
            while True:
                ...
                solve_idx = _prompt_next_task_gui(solve_options, env, color_map, selected_target, fps=120)
