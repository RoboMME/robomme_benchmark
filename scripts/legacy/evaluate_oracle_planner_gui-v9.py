...
def _prompt_next_task_gui(
    ...
    while True:
        ...
        banner_lines.extend(
            _wrap_text_line(
                "Click a solve button to arm it, pick a target on the segmentation view, then click the same button again to confirm. Number keys still trigger immediately; Space pause, q/Esc exit.",
                combined.shape[1] - 20,
            )
        )
        line_h = 22
