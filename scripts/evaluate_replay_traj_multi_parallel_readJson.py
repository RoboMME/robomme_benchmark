...
def _parse_args():
    ...
    )  # deprecated; kept for CLI compatibility
    parser.add_argument(
        "--max-workers",
        type=int,
        default=32,
...
def _replay_episode(
    ...
    metadata_path = Path(dataset_path).with_name(f"{Path(dataset_path).stem}_metadata.json")
    ...
    with h5py.File(dataset_path, "r") as dataset:
        ...
        env, episode_dataset, seed, difficulty_hint = resolver.make_env_for_episode(episode)
