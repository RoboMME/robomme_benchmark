...
@register_env("VideoChooseBin", max_episode_steps=2000)
class VideoChooseBin(BaseEnv):
    ...
    def evaluate(self,solve_complete_eval=False):
        ...
        tasks = [
            {
                        "func": lambda: static_check(self, timestep=int(self.elapsed_steps), static_steps=120),
                        "name": "Static",
                        "demonstration": True,
                        "failure_func": None,
                        "solve": lambda env, planner: solve_hold_obj(env, planner, static_steps=120),
                        },
            {
...
#HistoryBench
    def step(self, action: Union[None, np.ndarray, torch.Tensor, Dict]):
        ...
        swap_flat_two_lane(
            self,
            cube_a=self.target_cube_0,
            cube_b=self.target_cube_1,
            start_step=20,
            end_step=100,
            cur_step=timestep,
            lane_offset=0.1,
            smooth=True,
            keep_upright=True,
            cube_c=self.target_cube_2,  # Keep bin_2 in place to prevent collision during swap
        )
        swap_flat_two_lane(
            self,
            cube_a=self.bin_0,
            ...
            smooth=True,
            keep_upright=True,
            cube_c=self.bin_2,  # Keep bin_2 in place to prevent collision during swap
        )
