...
@register_env("HighlightPickube", max_episode_steps=2000)
class HighlightPickubeEnv(BaseEnv):
    ...
    def _load_scene(self, options: dict):
        ...
        # 使用 torch.random 和 HistoryBench_seed 随机选择一个 cube 作为 target
        if self.all_cubes:
            generator = torch.Generator()
            generator.manual_seed(self.HistoryBench_seed)
            ...
            print(f"Selected target cube: {self.target_cube_name}")

            # 保存所有非目标 cube，用于失败检测等用途
            self.non_target_cubes = [cube for idx, cube in enumerate(self.all_cubes) if idx != target_idx]
        else:
    ...
    def evaluate(self,solve_complete_eval=False):
        ...
        tasks = [
             {
            "func": lambda: static_check(self, timestep=int(self.elapsed_steps), static_steps=40),
            "name": "Static",
            "demonstration": True,
            "failure_func": None,
            "solve": lambda env, planner: solve_hold_obj(env, planner, static_steps=40),
             },
            {
                "func": lambda: is_obj_pickup(self, obj=self.target_cube),
                "name": f"Pick up highlight cube",
                "demonstration": False,
