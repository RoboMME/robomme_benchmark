...
def find_path_0_to_8(start, target, R=3, C=3, diagonals=False, generator=None, blocked_nodes=None):
    ...
    if not path:
        print(f"❌ Cannot find path from {start} to {target}!")
...
def run_path_generation(run_id, generator, segments, R=3, C=7, backtrack_enable=True):
    ...
    print(f"\n{'=' * 60}")
    print(f"Run {run_id}: Finding path through waypoints {' → '.join(map(str, waypoints))}")
    print(f"{'=' * 60}")
    ...
    for seg_idx, (seg_start, seg_target, seg_blocked) in enumerate(segments):
        ...
        print(f"  Path 1 (Forward {seg_start}→{seg_target}):", end=" ")
        ...
        if do_backtrack:
            ...
            print(f"  Path 2 (Backward {seg_target}→{seg_start}):", end=" ")
            ...
            print(f"  Path 3 (Forward {seg_start}→{seg_target}):", end=" ")
            ...
            print(f"  → Segment generated 3 paths (with backtracking)")
        else:
            # No backtracking: just use forward path
            seg_combined = path_forward
            seg_edges = edges_fwd

            print(f"  → Segment generated 1 path (no backtracking)")
...
def run_path_generation(run_id, generator, segments, R=3, C=7, backtrack_enable=False):
    """
    Run a single path generation with segments
