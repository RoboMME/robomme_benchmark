...
@register_env("InsertPeg", max_episode_steps=200)
class InsertPeg(BaseEnv):
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # historybench-v2.7/historybench/HistoryBench_env/PickPeg.py:243
            pose = self.peg.pose
            pose_p = np.asarray(pose.p, dtype=np.float32).reshape(-1).copy()
            pose_q = np.asarray(pose.q, dtype=np.float32).reshape(-1).copy()
            ...
            # if self.HistoryBench_seed<30:
            #     self.obj_flag=-1
            #     self.direction=1
            # elif self.HistoryBench_seed<60:
            #     self.obj_flag=-1
            #     self.direction=-1
            # elif self.HistoryBench_seed<90:
            #     self.obj_flag=1
            #     self.direction=1
            # #elif self.HistoryBench_seed<60:
            # else:
