...
class EpisodeConfigResolver:
    ...
    def __init__(
        ...
        if env_group not in self.dataset:
            raise KeyError(f"Dataset missing group '{env_group}'")
        self.env_dataset = self.dataset[env_group]
    ...
    def make_env_for_episode(self, episode: int):
        ...
        if difficulty_hint:
            ...
            print(f"[{self.env_id}] Episode {episode}: seed={seed}, difficulty={difficulty_hint}")
        else:
            print(f"[{self.env_id}] Episode {episode}: seed={seed}")
