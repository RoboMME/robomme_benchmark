...
def _resolve_h5_path(env_id: str, dataset_path: Union[str, Path]) -> Path:
    """
    Resolve h5 file path: if dataset_path is a full path to a .h5 file (suffix .h5), use it;
    otherwise treat as directory and use dataset_path/record_dataset_{env_id}.h5.
    """
    p = Path(dataset_path)
    if p.suffix == ".h5":
        return p
...
def list_episode_indices(env_id: str, dataset_path: Union[str, Path]) -> List[int]:
    """
    Open h5 at dataset_path (full path to .h5) or dataset_path/record_dataset_{env_id}.h5,
    read env_{env_id} keys, return sorted episode numbers, then close. Raises if file or group missing.
    """
    h5_path = _resolve_h5_path(env_id, dataset_path)
...
class EpisodeDatasetResolver:
    ...
    def __init__(self, env_id: str, episode: int, dataset_path: Union[str, Path]):
        self.env_id = env_id
        self.episode = episode
        self._h5_path = _resolve_h5_path(env_id, dataset_path)
