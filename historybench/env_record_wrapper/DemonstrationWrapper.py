...
- 不包含视频保存功能；reset 返回完整列表 (obs_list, reward_list, ...)；step 通过 _augment_obs_and_info 注入当前步的 frames、subgoal 等。
"""
import copy
import re
import time
...
class DemonstrationWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        """重置环境并生成演示轨迹，再执行一步初始动作后返回 obs/info（含完整列表 frames、subgoal 等）。"""
        ...
        self.episode_success = False
        # 生成演示轨迹，返回所有 obs_list, reward_list, terminated_list, truncated_list, info_list
        obs_list, reward_list, terminated_list, truncated_list, info_list = self.get_demonstration_trajectory()
        ...
        self.demonstration_data = (obs_list, reward_list, terminated_list, truncated_list, info_list)
